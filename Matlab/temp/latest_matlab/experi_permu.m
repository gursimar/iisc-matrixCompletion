...
% Control variables
r=5;           % rank approximation (or latent features)
...
load('Permutated_M.mat','M')    % PERMUTATED M LOAD......
...
for f=1:mc_itr
    ...
    while(FRO_NORM_DIFF_MISSING/FRO_NORM_DIFF_MISSING_OLD<0.9999 || ~k || ~(k-1))
    ...
    RMSE_missing(f) = sum(sum((M_approx_missing-M).^2  )) 
    toc
end
display ('done')
