...
% Control variables
r=60;           % rank approximation (or latent features)
l_lim=10;     % Missing enteries limit (starting from 1) for the loop 
lrate=0.002;    % learning rate
mc_itr = 1  % Monti carlo iterations
...
for f=1:mc_itr
	...
	% Approximation matrix for the original matrix
        M_approx_missing = u_approx_missing * v_approx_missing;r=60;
        r=60;           % rank approximation (or latent feat       % rank approximation (or latent feat
        ...
        while(FRO_NORM_DIFF_MISSING>fro_th)
            ...
            M_approx_missing=double(M_approx_missing);
            end
